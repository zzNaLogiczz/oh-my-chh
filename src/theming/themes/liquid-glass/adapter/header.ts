...
function decorateAccountLinks(accountRoot: Element | null, scope: EnhancementScope): void {
  if (!accountRoot) return;
  if (accountRoot.getAttribute("data-chh-lg-account-ready") === "true") {
    enhanceAccountUserGroupBadge(accountRoot, scope);
    return;
  }
...
function enhanceAccountUserGroupBadge(accountRoot: Element, scope: EnhancementScope): void {
  ...
  enhanceRankBadge(userGroupLink, scope, identity, rankLabel);
...
function rebuildBrandRail(headerInner: HTMLElement, scope: EnhancementScope): void {
  ...
  if (account?.parentNode) {
    ...
    decorateAccountLinks(account, scope);
  }
...
function buildHeaderShell(scope: EnhancementScope): void {
  ...
  rebuildBrandRail(headerInner, scope);
...
export function enhanceLiquidGlassHeader({ settings }: AdapterContext, scope: EnhancementScope): void {
